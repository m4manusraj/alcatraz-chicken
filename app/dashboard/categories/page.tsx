    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        ...
        const categoriesData = snapshot.docs.map((doc) => {
          const data = doc.data()
          console.log("Category data:", { id: doc.id, name: data.name, order: data.order })
          return {
            id: doc.id,
            ...data,
          }
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        {error && (
          <div className="bg-red-500/10 border border-red-500/20 text-red-500 p-4 rounded-lg mb-6">{error}</div>
        )}
      ...
      <div className="bg-blue-900/20 border border-blue-500/30 text-blue-400 p-4 rounded-lg">
        <p>Debug: Found {categories.length} categories</p>
        {categories.length > 0 && (
          <p>
            First category: {categories[0].name} (Order: {categories[0].order})
          </p>
        )}
      </div>
      ...
      <Card>
        {categories.length === 0 ? (
          <div className="p-6 text-center">
            <p className="text-gray-400">No categories found. Add your first category to get started.</p>
          </div>
        ) : (
          <Table>
            <TableHeader>
